...
from src.environments.pricing_env import PricingEnv
...
def run_episode(env, agent=None, random_policy=False):
    ...
    while not done:
        ...
        daily_stats['revenues'].append(reward)
...
def evaluate_agents(num_episodes=10):
    ...
    try:
        ...
        model_path = project_root / "models" / "ppo_pricing_agent.pth"
        trained_agent.load_state_dict(torch.load(str(model_path), map_location='cpu'))
        trained_agent.eval()
        print("✅ Loaded trained PPO agent")
    except FileNotFoundError:
        print("❌ No trained agent found. Please run 'python train_ppo.py' first.")
        return
...
def create_comparison_plot(random_stats, trained_stats, random_mean, trained_mean):
    """Create a comparison plot of agent performance"""
    
    fig, ((ax1, ax2), (ax3, ax4)) = plt.subplots(2, 2, figsize=(15, 10))
    fig.suptitle('🤖 PPO Agent vs Random Policy Comparison', fontsize=16, fontweight='bold')
    
    days = random_stats['days']
    
    # Plot 1: Daily Prices
    ax1.plot(days, random_stats['prices'], 'r-', alpha=0.7, linewidth=2, label='Random Policy')
    ax1.plot(days, trained_stats['prices'], 'b-', alpha=0.7, linewidth=2, label='PPO Agent')
    ...
    ax2.plot(days, trained_stats['signups'], 'b-', alpha=0.7, linewidth=2, label='PPO Agent')
    ...
    ax3.plot(days, trained_stats['revenues'], 'b-', alpha=0.7, linewidth=2, label='PPO Agent')
    ...
    categories = ['Random Policy', 'PPO Agent']
    ...
    plt.tight_layout()
    chart_path = project_root / "results" / "pricing_comparison.png"
    plt.savefig(str(chart_path), dpi=300, bbox_inches='tight')
    plt.show()
    
    print("\n📈 Visualization saved as 'results/pricing_comparison.png'")
